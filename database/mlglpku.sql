...
BEGIN
	SELECT module_category, count(*) as kount FROM module group by module_category order by kount desc, module_category;
END$$
...
BEGIN
    PREPARE statement FROM
      "SELECT   module_category, count(*) as kount
       FROM     module
       WHERE    module_title like ?
       OR 	module_leader like ?
		 group by module_category order by kount desc, module_category";
	SET @p1 = concat('%', inSearchString, '%') ;
	EXECUTE statement USING @p1, @p1;		 
END$$
...
BEGIN
	SELECT module_id, module_title, module_leader, module_description, module_image, module_category FROM module ORDER BY module_id;
END$$
...
BEGIN
	SELECT module_id, module_title, module_leader, module_description, module_image, module_category FROM module ORDER BY RAND() LIMIT 12;
END$$
